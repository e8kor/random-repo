...
trait Serializable[A, B] {

  def serialize(entity: A): B

}

trait SerializeToMap[T] extends Serializable[T, Map[String, String]]

object Serializable {

  implicit class MapSerializableOps[A: SerializeToMap](entity: A) {

    def serialize: Map[String, String] = {
      val ev = implicitly[SerializeToMap[A]]
      ev.serialize(entity)
    }

  }

  implicit object HeadingLocationDeserializer extends SerializeToMap[HeadingLocation] {
  ...
  implicit def optionSerializer[T: SerializeToMap]: SerializeToMap[Option[T]] = {
    new SerializeToMap[Option[T]] {

      override def serialize(entity: Option[T]): Map[String, String] = {
        entity.map(_.serialize).getOrElse(Map.empty[String, String])
      }

    }
  }

  implicit object LeavingLocationDeserializer extends SerializeToMap[LeavingLocation] {
  ...
  implicit object LocationDeserializer extends SerializeToMap[Location] {
  ...
  implicit object AirportDeserializer extends SerializeToMap[Airport] {
  ...
  implicit object RunawayDeserializer extends SerializeToMap[Runaway] {

    override def serialize(entity: Runaway): Map[String, String] = {

      entity.headingLocation.serialize ++ entity.leavingLocation.serialize ++ Map(
  ...
  implicit object CountryDeserializer extends SerializeToMap[Country] {
